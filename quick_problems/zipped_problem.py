...
if __name__=="__main__":
    ...
    marks_master_list = []
    for _ in range(X):
        # marks_per_student = map(float, input().split())
        marks_per_student.extend(map(float, input().split()))
        marks_master_list.append(marks_per_student)    
        marks_per_student = []

    # A = [1,2,3]
    # B = [6,5,4]
    # C = [7,8,9]
    Y = [marks_master_list[0]]
    for i in marks_master_list[1:]:
        Y = Y + [i]
